...
int main(int argc, char** argv) {
    ...
    if (opt->mode == compress) {
        string res = compress_stream(*in, *out, in_count, out_count);
        if (!res.empty()) {
            cerr << res << endl;
        }
    } else {
        string res = decompress_stream(*in, *out, in_count, out_count);
        if (!res.empty()) {
            cerr << res << endl;
        }
    }

    // Report the compression ratio on stderr
    if (opt->mode == compress) {
        cerr << "C";
    } else {
        cerr << "Dec";
    }
    cerr << "ompression ratio (in/out/ratio): " << in_count << " / " << out_count << " / " << (((double) out_count) / ((double) in_count)) << endl;
